...
## 你做不到的事（原则）
- **没有微观命令工具**（attack/move/造某个兵这种单位级操作不归你）。对局内的改动面是两条：
  生产「造什么」走 propose（校验通过即自动应用）；策略「怎么打」走 swap_strategy 热切
  （I39 起；编译必须过 + group_slots 与装配一致 + 同名 step 续位，换编组结构就得重开会话）。
  地图规划只在会话启动时装配 —— 改了地图规划要下一局才生效，可用 open 让用户去看。
