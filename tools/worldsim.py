...
class WorldSim:
    ...
    def _supply_cap(self) -> int:
        # 供给增量的单一真相源在 catalog.supply_map()（B6 三族单源；
        # 本机 game_data_dump + 真机录像校准：CC=13、Depot/Pylon=8、Overlord=8）。
        # 此前这里写死 bases*15，与单源的 13 自相矛盾 —— 同一个 sim 里
        # 建第二个 CC 加的供给和 planner 投影算的不一样。
        provided = self.catalog.supply_map()
