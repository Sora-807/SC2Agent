...
// N4/D3 决策锁：断言的是用户拍板的 UI/交互决策（十五/二十二轮等），不是实现细节。
// 重构允许同步改断言；不许静默删测试。读取经 source-scan 单点（剥注释、cwd 无关）。
...
describe("ChatDock：商量回路已接上", () => {
  it("不再有 B10 时代的禁用占位（placeholder 文案退役）", () => {
    expect(src).not.toContain("agent 接入见 B10");
    expect(src).not.toContain("disabled\n");
  });

  it("输入框禁用是有条件的（后端不在 / 有理由）—— **运行中不禁用**（2026-08-24 插话）", () => {
    expect(src).toMatch(/disabled=\{!api\.ok \|\| chatReason !== null\}/);
    expect(src).toContain("interjectChat(text)");   // busy 时回车 = 插话
  });
...
describe("ChatDock：真流式（2026-08-22 十五轮，接 BaseAgent start_stream）", () => {
  it("客户端有 SSE 流（/api/agent/chat/stream + 逐 data: 行解析 + 失败回退整段）", () => {
    ...
    // 旧后端/网络失败 → 回退整段接口：流函数体内调用整段 sayChat（N4 改锁代码事实，
    // 原断言靠行尾注释「// 回退」成对——真剥注释后断空）
    expect(c).toMatch(/sayChatStream[\s\S]*?sayChat\(text\)/);
  });
  ...
  it("第一个思考 token 即点亮思考行：reasoning 分片直通 ThinkRow（running 态）", () => {
    ...
    expect(src).not.toContain("Typewriter");   // 假流式打字机退役（真分片直出）
    expect(src).not.toContain("typing");       // typing 标记随 Typewriter 退役
  });
  ...
  it("发话即显「思考中」占位：首个分片到达前不是死屏（2026-08-24 token 流慢的 UI 半边）", () => {
    expect(src).toContain('live.length === 0');
    expect(src).toContain('<ThinkRow text="…" running');
  });
  ...
  it("A 批（2026-08-24）：segments 交错时间线 + 插话落 live 末尾", () => {
    ...
    expect(client).toContain("segments?: ChatSegment[]");
    expect(client).toContain("interjection?: boolean");
  });
  ...
  it("跟随提醒是系统条不是用户气泡（§0.52：对局没结束系统注入，不冒充用户消息）", () => {
    ...
    expect(client).toContain("nudge?: boolean");     // 契约面：ChatMessage 可带 nudge
  });
});
...
describe("插话排队条：送达即撤（2026-08-25 用户报「排队中一直挂着，发出去了也不消失」）", () => {
  it("根因接线：SSE interject_delivered（drain 时刻）→ 撤排队条 —— 不再只等 round/finally", () => {
    ...
    expect(client).toContain("interject_delivered");
    expect(src).toContain("interject_delivered");
    expect(src).toContain("withoutDelivered(q, ev.texts)");
  });
