...
依赖：无硬依赖，可独立做；与 I1（常驻监听）正交——那是推、这是拉快照。
估 0.5-1 天。
...
1. **observe 的 step 参数**——已经取消（批 4 改自动 step：≤14×14 全量、超出
   自动降密度并在输出尾部标注；工具 schema 无 step 键、函数体不读
   args.get("step")、提示词只说"step 自动"）。无遗留，不立 issue。
...
0.5. **[P1] I23 策略面缺口**（术语 flow→策略清扫 + 装配可视化 + 编写向导——遗留多年的半成品面；详见 I23 节）。
0. **[P1] I21 用户视窗上下文自动注入**（发消息时带 ui_context：页面/规划/地图/视窗/悬停——用户不想重复交代正在看什么；详见 I21 节）。
...
13. **[P1→已关闭（2026-08-23 §0.41）] 策略热改断层（I16）**——热切 V1 已落地：
    `POST /api/session/swap`（整份切换 + 同装配约束 + 同名 step 续位，offline/sim/sc2
    全支持 —— sc2 走控制文件通道）；转移历史记 swap 事件、observe 可见。
    决策留档 [`adr/0031-编译期模板展开.md`](adr/0031-编译期模板展开.md) 与 WORKLOG §0.41。
...
18. **[P1] 观察包"零收入检测"（2026-08-24 真机事故的 B 方案，用户拍板随后做）**——
    事故：真机出生右上，经济维持器把工人派去左下矿（根因已修：§0.44 出生点检测），
    但 **观察面完全看不出**："采矿实际 8/可达 8"（数的是被分配数，不是在产出），
    矿 50 不涨与"正常"并存，agent 无法区分"分配错/走错路/数据延迟"。
    方案（纯派生、不动契约核心）：观察包经济段加**资源产出速率**（近 N 秒 Δ矿/Δ气）+
    **采矿工人距目标矿脉的距离分布**；结构化警报「收入为 0 且有采矿分配 →
    疑似采错矿/路途过远」（I17 家族）。agent 侧 [I1] 改进笔记同源。

19. **[P2] 复盘（回放源）切换加载慢（2026-08-24 用户反馈，可后排）**——复盘页换源/
    拖时间轴时前端明显卡顿：回放路径整份 JSONL 重新拉取+逐行解析+全量重建 store，
    帧越多越慢（长对局 500+ 帧每帧带 world 全量单位）。候选优化：按需分窗解析
    （seek 只解目标窗口附近）、单位列表增量渲染、或后端给录制加索引（每 topic 的
    偏移量表）。用户拍板：性能问题不好解可以之后再说。
- [ ] repair 操作不在 OP_CATALOG（SCV 修理目标/跟随大部队）—— D3 立项，批 6 后排期
