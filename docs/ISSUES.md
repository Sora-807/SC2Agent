...
## I40 人族挂件转移（Lift/Land 接管孤儿挂件）六层全缺（用户 2026-08-25 发现并立项，拍板排真机复验后）

**机制事实（真机）**：建筑 `Lift` 升空（变飞行形态，挂件留在原地变**孤儿**但实体存活）→
另一建筑 `Land` 降落、落点让自身挂件格压住孤儿挂件 → 接管。**跨厂型有效**：兵营的
TechLab 可被工厂接管（实体类型不变，仍是 BARRACKSTECHLAB）。经典用途 = 快坦克：
工厂在建时先在兵营上并行造 TechLab，工厂完工落上去直接出坦克；反应堆同理跨厂转移。

**现状核实（2026-08-25，六层全空）**：

| 层 | 缺口 |
|---|---|
| driver | 无 Lift/Land 翻译器（dump 确认 `Lift` 能力在、飞行变体在 catalog variants；Land 带目标点，burnysc2 AbilityId 待查） |
| 队列词表 | QueueOp 闭集无升降空/转移 op |
| 放置 | `_addon_cells` 有挂件格几何，无「孤儿挂件检索 + 落点对准」 |
| 确认状态机 | build-flight 只认「新实体出现」；升空降落是**实体变形**（BARRACKS↔FLYING）+ 逐帧位移，需新确认形态 |
| 挂件归属 | **最深的坑**：runtime/planner 均按「挂件类型↔固定母建筑类型」建模（reactor_map/techlab_map 键=母类型、坦克科技门判 factorytechlab 在场）——跨厂接管打破类型级假设 |
| 仿真/模板 | build order 无 swap 语义；bio_tank V3 串行 `Build(factorytechlab)` |

**形态约束（立项拍板）**：不做通用自由飞行（微操域明确不做）——只做**结构化高层 op**
（如 `swap_addon(donor, receiver)`），运行时分解为 lift donor → land receiver@挂件格；
宏视角只关心「挂件归谁」，飞行过程是执行细节。

**分批**：
- **批 1（离线可验，含必还的债）**：挂件归属从类型级改实例级（跨厂接管的前提，
  无论如何该还）+ 语义 op + 仿真建模 + **catalog 挂件通用性错位修正**
  （terran/techlab 与 factorytechlab 现各绑各的母建筑——真机跨厂通用）；
- **批 2（真机）**：driver Lift/Land 翻译 + 变形确认状态机（挂件孤儿/接管对账）；
- **批 3**：模板/策略消费（bio_tank 改并行挂件版）。

**排序依据**：验证强依赖真机 → 排「真机一局验收 P0 五条」之后；对当前战术链是
优化项非阻塞项。关联：REFACTOR 无此条（功能非债）；I39 批 B 无关。
...
19. **live 投影窗口语义（原#10）——已拍板（2026-08-25）：until_complete + 封顶**——
    ...
    实测帧大小，超预期再回调。
20. **复盘（回放源）切换加载慢（原#19，用户拍板可后排）**——换源/拖时间轴前端卡顿：
    整份 JSONL 拉取+逐行解析+全量重建。候选：分窗解析/增量渲染/录制索引。
21. **repair / set_rally 操作（原尾注，D3 立项批 6 后排期）**——repair 不在 OP_CATALOG
    （SCV 修理目标/跟随大部队）；set_rally（集结点）为 I24 方案 3 的兜底项，可同批。
22. **I40 人族挂件转移（Lift/Land 接管孤儿挂件）——已立项（2026-08-25，拍板：排真机
    复验后）**——六层全缺（driver 能力/队列 op/放置对准/变形确认/挂件归属/仿真）；
    对当前战术链是优化非阻塞（bio_tank 串行造 factorytechlab 也能出坦克，只是慢）；
    详见 I40 节。
