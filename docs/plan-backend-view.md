...
**七条审批红线全部有测试**（`tests/api/test_proposals.py`）。最要紧的两条：
- **P2** 校验未通过的提案**照样存、照样可见**(agent 要学、用户要诊断),但 `accept` 返 409;
- **P5** `anchor` 超 `ANCHOR_STALE_SECONDS=60` 自动"已失效",禁止盲接受。
  (比命令的 `MAX_STALE_SEQ=5` 宽松得多 —— 提案是给人看的,看几十秒是正常的。)

**双投影预览成立了**(§6 认定的杀手功能)。它依赖前面刚修的队列投影桥,
两条曲线用**同一个 GameState** 起点,所以差异只来自队列本身。实测:
当前顺序 `[重工厂, 补给站]` → 终点 `supply_cap=15`、什么都没建成(卡在缺前置的重工厂);
提案后 `[补给站, 重工厂]` → 终点 `supply_cap=23`、补给站建成。

**hunk 是可应用的操作**(insert/delete/modify/reorder)而不是自由文本,所以"部分接受"能真的执行。
`apply_hunks` 刻意**按顺序逐个应用**而不是先算总位移 —— 部分接受时下标语义才和用户看到的一致。
应用走**与 agent 相同的命令路径**(`session.queue_op("submit", ...)`),不开 UI 后门(P4)。

**存储用追加日志**:提案的价值一半在历史(agent 提过什么、为什么被拒),追加天然保留;
重载时取每个 id 的最后一条状态。

**踩坑两处**:
1. 双投影一开始两条曲线一模一样 —— 查下去是**测试场景选错了**:沙盒开局只有指挥中心+农民,
   重工厂缺前置、机枪兵缺产出建筑,两种顺序都立刻卡住。换成"补给站 vs 重工厂"才有区别。
   这也说明预览在"两边都卡死"的情况下确实该显示成一样(不是 bug)。
2. `session/stop` 没断开 `proposals.session` → 停会话后新建的提案会基于**死会话**的世界算 anchor
   (拿到一个永远不变的 game_time),P5 的失效判断就永远不触发。已修 + 测试。

**`proposals` 帧只在内容变化时发**(指纹 = id+状态):提案是事件驱动的,每帧一条会把 WS 刷满。
