...
> ISSUES.md 的 I17/I19 是它的开放分项（开放任务清单 #8/#10，2026-08-25 重排）；
> I18/I20 已落地关闭（见 ISSUES-ARCHIVE.md）；本档持有全景、
> 反模式、目标架构与接法取舍。
...
- **常驻监听（原开放清单 #1，2026-08-25 核对已关闭）**：常驻循环 = 本档闭环的"运行态"，已由
  sleep 工具 + 对局跟随 + 警报唤醒 + 节流建成（§0.45/§0.47/§0.55，见 ISSUES-ARCHIVE.md）。
  读面（I18/I20 已落地；I17 剩余）与记忆（I19 效果观察中）到位情况直接影响常驻 Agent 的
  空转率——I25 事故（假警报反复叫醒）即是反例。
